...
function indicatorInit(navWrapper, indicatorName, navItems) {
    ...
    function indicatorHandler(el) {
        /* Remove active status from all navigation link */ items.forEach((item)=>{
            item.classList.remove("active");
            item.removeAttribute("style");
        });
        if (nav.classList.contains("nav1")) {
            /* Horizontal move */ indicator.style.width = `${el.offsetWidth}px`;
            indicator.style.left = `${el.offsetLeft}px`;
            /* Changes color of indicator by attribute data-color */ indicator.style.backgroundColor = el.getAttribute("data-color");
        } else {
            /* Vertical move */ indicator.style.height = `${el.offsetHeight}px`;
            indicator.style.top = `${el.offsetTop}px`;
            /* Changes color of indicator by attribute data-color */ indicator.style.backgroundColor = el.getAttribute("data-color");
        }
        /* Changes color of text by attribute data-color */ el.classList.add("active");
